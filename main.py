...
def ouvrir_ajout_bien_immobilier():
    ...
    type_immobilier_label = tk.Label(
        ajout_bien_immobilier_window, text="Type d'immobilier :"
    )
    ...
    type_immobilier_maison_radio = tk.Radiobutton(
        ...
        value="Maison",
        command=lambda: superficie_jardin_entry.grid(),
    )
    ...
    type_immobilier_appartement_radio = tk.Radiobutton(
        ...
        variable=type_immobilier_value,
        value="Appartement",
        command=lambda: superficie_jardin_entry.grid_remove(),
    )
    ...
    adresse_type_voie_label = tk.Label(
        ajout_bien_immobilier_window, text="Type de voie:"
    )
    ...
    adresse_type_voie_dropdown = tk.OptionMenu(
        ajout_bien_immobilier_window, adresse_type_voie_var, *addresse_type_voie_choices
    )
    adresse_type_voie_dropdown.place(x=298, y=50)

    adresse_nomvoie_label = tk.Label(
        ajout_bien_immobilier_window, text="Nom de la voie:"
    )
    ...
    superficie_couvert_label = tk.Label(
        ajout_bien_immobilier_window, text="Superficie couverte (m²) :"
    )
    ...
    superficie_couvert_entry = tk.Entry(
        ajout_bien_immobilier_window,
        validate="key",
    ...
    superficie_jardin_label = tk.Label(
        ajout_bien_immobilier_window, text="Superficie jardin (m²) :"
    )
    superficie_jardin_label.grid(row=4, column=0, padx=10, pady=10, sticky=tk.W)
    validate_func = ajout_bien_immobilier_window.register(validate_input)
    global superficie_jardin_entry
    superficie_jardin_entry = tk.Entry(
        ajout_bien_immobilier_window,
        validate="key",
        validatecommand=(validate_func, "%P"),
    )
    ...
    nombre_pieces_label = tk.Label(
        ajout_bien_immobilier_window, text="Nombre de pièces :"
    )
    nombre_pieces_label.grid(row=5, column=0, padx=10, pady=10, sticky=tk.W)
    validate_func = ajout_bien_immobilier_window.register(validate_input)
    global nombre_pieces_entry
    nombre_pieces_entry = tk.Entry(
        ajout_bien_immobilier_window,
        validate="key",
        validatecommand=(validate_func, "%P"),
    )
    ...
    classe_energetique_label = tk.Label(
        ajout_bien_immobilier_window, text="Classe énergétique :"
    )
    ...
    classe_energetique_dropdown = tk.OptionMenu(
        ajout_bien_immobilier_window,
        classe_energetique_var,
        *classe_energetique_choices,
    )
    ...
    annee_construction_label = tk.Label(
        ajout_bien_immobilier_window, text="Année de construction :"
    )
    annee_construction_label.grid(row=7, column=0, padx=10, pady=10, sticky=tk.W)
    validate_func = ajout_bien_immobilier_window.register(validate_input)
    global annee_construction_entry
    annee_construction_entry = tk.Entry(
        ajout_bien_immobilier_window,
        validate="key",
        validatecommand=(validate_func, "%P"),
    )
    ...
    nature_gestion_label = tk.Label(
        ajout_bien_immobilier_window, text="Nature de la gestion :"
    )
    ...
    date_mise_marche_label = tk.Label(
        ajout_bien_immobilier_window, text="Date de mise en marche :"
    )
    ...
    day_spinbox_value = tk.Spinbox(
        ajout_bien_immobilier_window, from_=1, to=31, width=2
    )
    day_spinbox_value.place(x=170, y=400)

    global month_spinbox_value
    month_spinbox_value = tk.Spinbox(
        ajout_bien_immobilier_window, from_=1, to=12, width=2
    )
    month_spinbox_value.place(x=200, y=400)

    global year_spinbox_value
    year_spinbox_value = tk.Spinbox(
        ajout_bien_immobilier_window, from_=1800, to=2023, width=4
    )
    ...
    valider_button = tk.Button(
        ajout_bien_immobilier_window, text="Valider", command=valider_saisie
    )
...
def recup_data_in_db():
    ...
    cursor.execute(
        "SELECT ID, type_immobilier, rue_complete, cp_adresse, nom_ville_adresse, superficie_couvert, superficie_jardin, nombre_pieces, classe_energetique, annee_construction, nature_gestion, prix, timestamp FROM biens_immobiliers"
    )
...
def tableau_infos_bien():
    ...
    table["columns"] = [
        ...
        "prix",
        "timestamp",
    ]  # E231: 'prix','timestamp'] -> 'prix', 'timestamp']
    table["show"] = "headings"
...
def ouvrir_suppression_bien_immobilier():
    ...
    select_all_button = tk.Button(
        suppression_bien_window,
        text="Sélectionner tout",
        command=lambda: id_list.select_set(0, tk.END),
    )
    ...
    deselect_all_button = tk.Button(
        ...
        command=lambda: id_list.selection_clear(0, tk.END),
    )
    ...
    delete_button = tk.Button(
        suppression_bien_window,
        text="Supprimer",
        command=lambda: supprimer_biens_selectionnes(id_list.curselection()),
    )
...
def supprimer_biens_selectionnes(selection):
    if not selection:
        messagebox.showwarning(
            "Aucune sélection",
            "Veuillez sélectionner au moins un bien immobilier à supprimer.",
        )
        return

    confirmation = messagebox.askyesno(
        "Confirmation de suppression",
        "Êtes-vous sûr de vouloir supprimer les biens immobiliers sélectionnés ?",
    )
    if not confirmation:
        return
    ...
    messagebox.showinfo(
        "Suppression réussie",
        "Les biens immobiliers sélectionnés ont été supprimés avec succès.",
    )
...
if not os.path.isfile(db_file):
    if messagebox.askyesno(
        ...
        "La base de données n'existe pas encore, voulez-vous la créer ?",
    ):
...
ajouter_bien_button = tk.Button(
    bouton_cadre,
    text="Ajouter un bien immobilier",
    command=ouvrir_ajout_bien_immobilier,
)
ajouter_bien_button.pack(side=tk.RIGHT)

# Bouton pour supprimer un bien immobilier
supprimer_bien_button = tk.Button(
    bouton_cadre,
    text="Suppression de biens",
    command=ouvrir_suppression_bien_immobilier,
)
...
vente_checkbox.pack()
location_checkbox = tk.Checkbutton(
    filters_frame, text="Location", variable=location_var
)
...
appartement_checkbox = tk.Checkbutton(
    filters_frame, text="Appartement", variable=appartement_var
)
